...
var semantics = grammar.createSemantics().addOperation('toHTML', {
    ...
    block_noformat: function (contents) {
        return '<pre>' + contents.toHTML() + '</pre>';
    },
    ...
    simpleWord_link1: function (_lb, txt, _bar, _s1, uri, _s2, _rb) {
        return curops.formatLink(uri.sourceString, txt.toHTML());
    },
    simpleWord_link2: function (_lb, uri, _rb) {
        var inner = escapeHTML(uri.sourceString);
        return curops.formatLink(uri.sourceString, inner);
    },
    simpleWord_user: function (_lb, user, _rb) {
        return '@' + escapeHTML(user.sourceString);
    },
    simpleWord_formatted: function (sp, fmt) {
        return sp.toHTML() + nodesToHTML(fmt, '');
    },
    simpleWordUntil_multiple: function (first, rest) {
        return first.toHTML() + nodesToHTML(rest, '');
    },
    simpleWordUntil_simple: function (contents) {
        return nodesToHTML(contents, '');
    },
    simpleWord_monospace: function (_l, contents, _r) {
        return '<code>' + contents.toHTML() + '</code>';
    },
    formatted_citation: function (_l, contents, _r) {
        return '<cite>' + nodesToHTML(contents, '') + '</cite>';
    },
    formatted_strong: function (_l, contents, _r) {
        return '<b>' + contents.toHTML() + '</b>';
    },
    formatted_emphasis: function (_l, contents, _r) {
        return '<i>' + contents.toHTML() + '</i>';
    },
    formatted_subscript: function (_l, contents, _r) {
        return '<sub>' + contents.toHTML() + '</sub>';
    },
    formatted_superscript: function (_l, contents, _r) {
        return '<sup>' + contents.toHTML() + '</sup>';
    },
    formatted_deleted: function (_l, contents, _r) {
        return '<del>' + contents.toHTML() + '</del>';
    },
    formatted_inserted: function (_l, contents, _r) {
        return '<ins>' + contents.toHTML() + '</ins>';
    },
    bareurl: function (_http, _s, _sep, _url) {
